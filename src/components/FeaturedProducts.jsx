...
//   const [heartIcon, setHeartIcon] = useState(Array(product.length).fill(false));
...
//     try {
//       const res = await axios.get("http://localhost:8000/product/all-products");
...
//         userId: 1, // dummy user – login होने पर dynamic कर देना
...
//           {(showAllProducts ? product : product.slice(0, 8)).map(
//             (product, index) => {
//               return (
//                 <div
//                   key={index}
...
//                     <Image
//                       src={product.images?.[0]} // ⬅️ first image
//                       alt={product.title}
//                       fill={false}
//                       width={0}
//                       height={0}
...
//                         className="cursor-pointer rounded-lg bg-[#B8DDB4]/45 p-1 h-7 sm:text-sm text-xs font-medium font-inter sm:max-w-[67px] w-full flex items-center justify-center gap-1 hover:bg-light-green hover:text-white active:bg-light-green active:text-white focus:bg-light-green focus:text-white transition-all duration-300 group"
//                       >
//                         <Icons
//                           icon="cartIcon"
